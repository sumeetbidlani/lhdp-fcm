...
export default function ClosedComplaints() {
  ...
  useEffect(() => {
    const fetchComplaints = async () => {
      ...
      try {
        ...
        if (data.closed_complaints) {
          setComplaints(data.closed_complaints);
          setFiltered(data.closed_complaints);
        } else {
          toast.error('Failed to load closed complaints');
        }
      } catch (error) {
        console.error('Fetch error:', error);
        toast.error('Error loading closed complaints');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="p-6 min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-white">
      {/* Title */}
      <h2 className="text-xl font-semibold mb-6">Closed Complaint/Feedback</h2>

      {/* Table */}
      {filtered.length === 0 ? (
        <p className="text-center mt-4">No closed complaints found.</p>
      ) : (
              ...
              {currentComplaints.map((complaint) => (
                <tr key={complaint.id} className="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <td className="px-4 py-2 flex space-x-2">
                    ...
                    {/* <Link href={`/dashboard/download-pdf/${complaint.id}`}>
